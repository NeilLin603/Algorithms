...
int main() {
    ...
    Node_t *fruitList;
    Fruit_t fruit, fruitPre = {.weight = 0};
    int count = 1;

    printf("The max price of backpack = %d\n",
           maxPrice(fruits, sizeof(fruits) / sizeof(fruits[0]), weightLimit, &fruitList));
    printf("-----------------------------------------\n");
    printf("%-*.*s%-*.*s%-*.*s%-*.*s\n", 12, 4, "Name", 12, 6, "Weight", 12, 5, "Price", 12, 5, "Count");
    printf("-----------------------------------------\n");
    while (popNode(&fruitList, &fruit)) {
        if (fruit.weight == fruitPre.weight) {
            count++;
        } else {
            if (fruitPre.weight) {
                printf("%-*.*s%-12d%-12d%-12d\n", 12, 10, fruitPre.name, fruitPre.weight, fruitPre.price, count);
            }
            fruitPre = fruit;
            count = 1;
        }
    }
    printf("%-*.*s%-12d%-12d%-12d\n", 12, 10, fruit.name, fruit.weight, fruit.price, count);
